...
- 🌊 **深海 ABYSS 默认视觉（v1.4）**：黑蓝深海层级底色 + 生物荧光青蓝 + **玻璃拟态**（半透明面板/发丝高光）+ 双层**海雪粒子**视差 + **ASCII 原子像素**动画（启动页水印 / 加载遮罩 / 离线页 / 终端首开占位，24 帧无缝循环）
- 🎨 **皮肤主题**：深海（默认）/ 石墨蓝 / 极光紫 / 翡翠 / 琥珀 五套预设，token 化即时预览；旧版「深蓝」自动迁移为深海，石墨蓝完整保留 v1.3 观感
- 😴 **离线页**：5 秒自动重试 + 「一键启动服务」（托管模式下直接拉起 DSH）
- 🔗 **外部链接**：页面中的外部链接自动用系统浏览器打开，下载文件自动存入下载目录
- 🪟 **窗口记忆**：记住窗口位置与最大化状态，关闭后原样恢复
- 🔑 单实例运行
- 📬 **未读消息感知**：主窗收进托盘后，任务栏红色角标 + 托盘「N 条未读」+ 悬浮鲸呼吸灯加速；看向窗口自动清零
- 🔍 **页内查找**：`Ctrl`+`F` 在 DSH 页面内即时查找，n/N 计数、上下跳转
- ⌘ **命令面板**：`Ctrl`+`K` 模糊搜索 25+ 动作——切皮肤、开关终端、服务启停、导入导出…一键盘唤出
- 🟦 **托盘状态色**：托盘鲸鱼图标带状态点（在线青蓝 / 尝试中琥珀 / 离线灰红），一眼可辨
- ⌨️ **全局快捷键呼出**：`Ctrl`+`Alt`+`J` 在任何界面一键呼出 / 隐藏主窗（设置可关）
- 🌙 **开机自启静默启动**：自启时小鲸鱼在托盘待命，不弹窗打扰
- 🔔 **服务状态系统通知**：主窗在托盘时，DSH 服务上线 / 掉线弹系统通知，点击直达主界面
- 💾 **设置导入 / 导出**：一键备份配置 JSON 并迁移到其他电脑（白名单校验，不含窗口位置与开机自启）
...
> 视觉语言「深海 ABYSS」（v1.4 起）：黑蓝深渊层底色 `#060B14→#10203A`、生物荧光青蓝 `#4CC2FF`、玻璃拟态 + 旋转渐变边框、双层海雪粒子、ASCII 原子像素动画。
...
| 按键 | 功能 |
| --- | --- |
| `Ctrl` + `K` | 命令面板：模糊搜索执行全部动作（切皮肤/开关终端/服务操作…） |
| `Ctrl` + `F` | 页内查找（高亮 + n/N 计数，Enter/Shift+Enter 跳转） |
| `Ctrl` + `Alt` + `J`（全局） | 任何界面呼出 / 隐藏 JUNDSH 主窗（设置可关） |
| `Ctrl` + `=` / `-` | 缩放界面 |
| `Ctrl` + `0` | 恢复 100% 缩放 |
| `` Ctrl `` + `` ` `` | 打开/关闭终端抽屉 |
| `Ctrl` + `L`（终端内） | 清空终端输出 |
