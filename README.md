...
- ⌨️ **内置终端（v1.3）**：标题栏按钮或 `Ctrl`+`` ` `` 快捷呼出 PowerShell 终端抽屉（IPC 串流，零原生依赖）
- 🐋 **桌面悬浮鲸鱼（v1.3）**：官方黑色鲸鱼标识悬空于桌面，可拖动/置顶，单击弹出功能菜单（实时连接状态），设置可开关
- 🎨 **皮肤主题（v1.3）**：深蓝 / 极光紫 / 翡翠 / 琥珀 四套预设，token 化即时预览
